...
if exist "%tauri_clean_script%" (
  ...
  if exist "%app_dir%\src-tauri\runtime" rd /s /q "%app_dir%\src-tauri\runtime"
  if exist "%app_dir%\src-tauri\target\release" rd /s /q "%app_dir%\src-tauri\target\release"
  if exist "%app_dir%\src-tauri\target" rd /s /q "%app_dir%\src-tauri\target"
...
:remove_pycache_dirs
echo [CACHE] Removing Python __pycache__ folders...
set "removed_pycache_count=0"
for /d /r "%repo_root%" %%D in (__pycache__) do (
  rd /s /q "%%~fD" >nul 2>&1
  if not exist "%%~fD\" set /a removed_pycache_count+=1
)
if "%removed_pycache_count%"=="0" (
  echo [INFO] No __pycache__ folders found.
) else (
  echo [SUCCESS] Removed %removed_pycache_count% __pycache__ folder^(s^).
)
exit /b 0
