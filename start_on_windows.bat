...
set "TMP_HELPER_ID=%RANDOM%_%RANDOM%"
set "TMPDL=%TEMP%\fairs_%TMP_HELPER_ID%_download.ps1"
set "TMPEXP=%TEMP%\fairs_%TMP_HELPER_ID%_expand.ps1"
set "TMPTXT=%TEMP%\fairs_%TMP_HELPER_ID%_txt.ps1"
set "TMPFIND=%TEMP%\fairs_%TMP_HELPER_ID%_find_uv.ps1"
set "TMPVER=%TEMP%\fairs_%TMP_HELPER_ID%_pyver.ps1"
set "TMPPIDPATH=%TEMP%\fairs_%TMP_HELPER_ID%_pid_path.ps1"
set "TMPHEALTH=%TEMP%\fairs_%TMP_HELPER_ID%_health.ps1"
...
for /L %%i in (1,1,60) do (
  for %%U in ("!BACKEND_BASE_URL!/api/health" "!BACKEND_BASE_URL!/health" "!BACKEND_BASE_URL!/docs" "!BACKEND_BASE_URL!/") do (
    set "backend_probe_result="
    for /f "usebackq delims=" %%R in (`powershell -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%TMPHEALTH%" %%~U`) do set "backend_probe_result=%%R"
    if /i "!backend_probe_result!"=="ok" goto :backend_ready_check
  )
  timeout /t 1 /nobreak >nul 2>&1
)
