...
The Settings endpoints persist to `runtime-settings.json` through `SettingsService`. Nested unknown fields, invalid roulette ranges, out-of-range polling values, empty JIT backend values, database configuration, environment values, and per-training `TrainingConfig` fields are rejected. Enabling JIT also requires runtime capability support; the supported Python 3.13 launcher runtime accepts it when `torch.compile` is available, while Windows rejects the `inductor` backend because Triton is unavailable and manually selected Python 3.14+ runtimes return `422` before persistence. Existing runtime files without the roulette block remain valid and receive backend defaults. Job and device changes are propagated to the live `TrainingService`; roulette rules are read from the same persisted runtime authority when new training data is prepared and when live inference steps are validated.
