...
FAIRS uses one active runtime file: `FAIRS/settings/.env`.

- Local mode: run directly on host without Docker.
- Cloud mode: run with Docker (backend + frontend).
- Mode switching: replace values in `FAIRS/settings/.env` only.
- No business-logic branching is required for runtime mode changes.

## 2. Runtime Profiles

- `.env.local.example`: local defaults (loopback host values, embedded DB).
- `.env.cloud.example`: cloud defaults (container bind host values, external DB).
- `FAIRS/settings/.env`: active profile used by launcher, tests, and Docker runtime env loading.
- `configurations.json`: non-runtime defaults only (no database runtime settings).
...
- Frontend dependency graph is lockfile-backed via `package-lock.json` and installed with `npm ci`.
