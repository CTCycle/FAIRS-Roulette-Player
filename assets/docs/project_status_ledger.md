...
The snapshot was bootstrapped from the `v3.4.2` checkout at baseline commit `25e2537`. The working tree was clean before this documentation-only change. The first test-runner attempt was blocked because the application had been uninstalled; the authorized Development reinstall restored the managed runtime, dependencies, Playwright browsers, frontend build, and SQLite database. The current validation result is `201 passed, 10 skipped, 1 warning`; a focused direct launcher/browser smoke is recorded as `PASS` in [`assets/QA/startup-launch-smoke-20260920.md`](../QA/startup-launch-smoke-20260920.md). No application capability is promoted to `VALIDATED` from source presence or test definitions alone.

The ten pytest skips were explicit: five PostgreSQL cases without `TEST_POSTGRES_URL`, four checkpoint-backed inference cases without a usable fixture, and one unsupported-Python JIT case. The runner separately reported frontend unit and frontend E2E phases as `SKIPPED` because those npm scripts do not exist. The focused launcher smoke is documented under `assets/QA/`; this ledger keeps the current result compact and links to the runner, test sources, and the smoke note.
...
| `application.startup` | `VALIDATED` | Windows launcher preparation, database startup, backend health, frontend readiness, single-worker lifecycle, and stop behavior. | Standard runner, direct launcher option 1, exact backend-stop simulation with the frontend still serving, option 13 process cleanup, delayed startup browser inspection, automatic readiness transition, retry/reload flows, supported viewport checks, reduced-motion checks, production build, and the current direct launcher/browser smoke all passed. | Browser opening remains best-effort by design; the runner's standalone frontend unit/E2E phases remain skipped because those npm scripts do not exist. | — | 2026-09-20 | `manual + E2E` | [`startup.md`](runtime/startup.md), [`deployment.md`](runtime/deployment.md), [`test_app_entrypoint.py`](../../app/tests/unit/test_app_entrypoint.py), [`app_flow.py`](../../app/tests/e2e/test_app_flow.py), [`../QA/startup-launch-smoke-20260920.md`](../QA/startup-launch-smoke-20260920.md) | Re-run the launcher smoke and startup browser slice after future launcher or bootstrap changes. |
| `backend.api-contracts` | `VALIDATED` | FastAPI routers, Pydantic contracts, health/system, upload, training, inference, settings, and generated frontend transport types. | Standard suite: `201 passed`; generated frontend contract check passed; route-specific API and browser tests passed; Ruff passed. | Full live inference-session routes remain outside this result because four tests skipped without a checkpoint/data fixture; PostgreSQL is tracked separately. | — | 2026-09-20 | `unit + E2E` | [`backend_api.md`](architecture/backend_api.md), [`execution_and_data_flow.md`](architecture/execution_and_data_flow.md), [`test_generated_contracts.py`](../../app/tests/unit/test_generated_contracts.py) | Re-run the contract and affected route slices after API or schema changes. |
| `backend.training` | `VALIDATED` | Dataset preparation, training validation/start/status/stop/resume, worker lifecycle, metrics, and checkpoint production. | Standard suite covered training start, status, stop, cancellation, resume, unit/service behavior, and data contracts; all executed cases passed. | No failure observed; compute-heavy runtime and checkpoints should be rechecked after learning or worker changes. | — | 2026-09-20 | `unit + E2E` | [`execution_and_data_flow.md`](architecture/execution_and_data_flow.md), [`workflows.md`](operations/workflows.md), [`test_training_api.py`](../../app/tests/e2e/test_training_api.py) | Repeat the training E2E workflow after changes to workers, checkpoints, data preparation, or training contracts. |
...
| `persistence.sqlite` | `VALIDATED` | SQLite default database, Alembic initialization, repositories, dataset/inference persistence, and checkpoint references. | Development reinstall initialized the database to Alembic head; SQLite persistence contract, migration rejection/rollback/concurrency, repository, and ORM tests passed; Alembic head check passed. | Non-empty unversioned databases, schema drift, and unknown revisions intentionally fail closed. | — | 2026-09-20 | `unit + E2E` | [`persistence.md`](architecture/persistence.md), [`test_database_initialization.py`](../../app/tests/unit/test_database_initialization.py), [`test_sqlite_repository_orm.py`](../../app/tests/unit/test_sqlite_repository_orm.py) | Re-run the SQLite contract after model, migration, or repository changes. |
...
| `runtime.settings` | `VALIDATED` | Typed `.env` configuration, `runtime-settings.json`, strict updates/reset, JIT capability checks, and live propagation. | Settings unit/API tests passed for structured persistence, partial update, reload/reset, supported JIT, and Windows `inductor` rejection. One unsupported-Python scenario was intentionally skipped on the managed Python 3.13 runtime. | Python 3.14+ rejection is not exercised locally in this run; the supported launcher runtime is 3.13. | — | 2026-09-20 | `unit + E2E` | [`configuration.md`](runtime/configuration.md), [`test_settings_system.py`](../../app/tests/unit/test_settings_system.py), [`test_settings_api.py`](../../app/tests/e2e/test_settings_api.py) | Re-run settings tests after configuration, capability, or propagation changes; use an alternate runtime only when that path is in scope. |
| `frontend.workflows` | `PARTIAL` | React/Vite shell, training and inference pages, guidance, settings UI, transport parsing, and desktop-width behavior. | Browser tests for navigation, page rendering, guidance, settings, upload/API flows, startup recovery/retry/reload, supported viewport containment, and reduced motion passed; frontend lint, production build, and manual visual inspection passed. | Four live inference-session tests skipped without a checkpoint/data fixture; `ISSUE-003` remains for absent standalone frontend unit/E2E npm scripts; the broader live inference UI still needs a fixture-backed review. | — | 2026-09-20 | `manual + E2E` | [`experience.md`](ui/experience.md), [`components_and_patterns.md`](ui/components_and_patterns.md), [`app_flow.py`](../../app/tests/e2e/test_app_flow.py), [`app/client/README.md`](../../app/client/README.md) | Revalidate the live inference UI and broader visual/accessibility paths after UI changes. |
| `qa.standard-runner` | `VALIDATED` | Repository-standard dependency setup, live services, Python tests, and optional frontend test phases. | `app/tests/run_tests.bat` previously completed with `Live server phase: PASS`, `Python tests: PASS`, `Frontend bootstrap: PASS`, `Frontend unit tests: SKIPPED`, and `Frontend E2E tests: SKIPPED`; the current equivalent repository-local run reported `201 passed, 10 skipped, 1 warning in 66.31s`, with frontend lint and production build also passing. | The skipped phases and tests are explicit validation debt, not hidden passes; the warning is a dependency deprecation warning. A fresh exact batch-runner retry was blocked before collection by access denied in the existing `runtimes/cache/uv` cache. | — | 2026-09-20 | `unit + E2E` | [`testing_and_quality.md`](coding/testing_and_quality.md), [`startup.md`](runtime/startup.md), [`run_tests.bat`](../../app/tests/run_tests.bat) | Restore writable canonical cache permissions and rerun the exact standard command; investigate any new skip, failure, or warning. |
...
| `runtime.settings` | Medium | Unsupported-Python JIT rejection path if alternate-runtime support remains in scope. | Low |
