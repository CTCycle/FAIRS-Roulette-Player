...
The snapshot was bootstrapped from the `v3.4.2` checkout at baseline commit `25e2537`. The working tree was clean before this documentation-only change. The first test-runner attempt was blocked because the application had been uninstalled; the authorized Development reinstall restored the managed runtime, dependencies, Playwright browsers, frontend build, and SQLite database. The current validation result is `195 passed, 10 skipped, 1 warning`; no application capability is promoted to `VALIDATED` from source presence or test definitions alone.
...
| --- | --- |
| `VALIDATED` | `docs.ontology`, `backend.api-contracts`, `backend.training`, `persistence.sqlite`, `runtime.settings`, `qa.standard-runner` |
| `WORKING` | `runtime.source-release` |
| `PARTIAL` | `application.startup`, `backend.inference`, `frontend.workflows` |
...
| `application.startup` | `PARTIAL` | Windows launcher preparation, database startup, backend health, frontend readiness, single-worker lifecycle, and stop behavior. | Development reinstall, frontend build, database initialization, standard-runner live service startup, health checks, and browser page loads passed. Unit coverage also passed. | Direct launcher option 1 browser-opening behavior and explicit option 13 process-stop smoke were not run separately; browser opening remains best-effort by design. | — | 2026-09-20 | `manual + E2E` | [`startup.md`](runtime/startup.md), [`deployment.md`](runtime/deployment.md), [`test_app_entrypoint.py`](../../app/tests/unit/test_app_entrypoint.py) | Run one direct launcher start/stop smoke after future launcher changes. |
| `backend.api-contracts` | `VALIDATED` | FastAPI routers, Pydantic contracts, health/system, upload, training, inference, settings, and generated frontend transport types. | Standard suite: `195 passed`; generated frontend contract check passed; route-specific API and browser tests passed; Ruff passed. | Full live inference-session routes remain outside this result because four tests skipped without a checkpoint/data fixture; PostgreSQL is tracked separately. | — | 2026-09-20 | `unit + E2E` | [`backend_api.md`](architecture/backend_api.md), [`execution_and_data_flow.md`](architecture/execution_and_data_flow.md), [`test_generated_contracts.py`](../../app/tests/unit/test_generated_contracts.py) | Re-run the contract and affected route slices after API or schema changes. |
...
| `frontend.workflows` | `PARTIAL` | React/Vite shell, training and inference pages, guidance, settings UI, transport parsing, and desktop-width behavior. | Browser tests for navigation, page rendering, guidance, settings, upload/API flows passed; frontend lint and production build passed. | Four live inference-session tests skipped without a checkpoint/data fixture; `ISSUE-003` remains for absent standalone frontend unit/E2E npm scripts; manual visual/accessibility review was not part of this run. | — | 2026-09-20 | `E2E` | [`experience.md`](ui/experience.md), [`components_and_patterns.md`](ui/components_and_patterns.md), [`app_flow.py`](../../app/tests/e2e/test_app_flow.py), [`app/client/README.md`](../../app/client/README.md) | Revalidate the live inference UI and perform manual visual/accessibility checks after UI changes. |
| `qa.standard-runner` | `VALIDATED` | Repository-standard dependency setup, live services, Python tests, and optional frontend test phases. | `app/tests/run_tests.bat` completed with `Live server phase: PASS`, `Python tests: PASS`, `Frontend bootstrap: PASS`, `Frontend unit tests: SKIPPED`, and `Frontend E2E tests: SKIPPED`; pytest reported `195 passed, 10 skipped, 1 warning in 66.88s`. | The skipped phases and tests are explicit validation debt, not hidden passes; the warning is a dependency deprecation warning. | — | 2026-09-20 | `unit + E2E` | [`testing_and_quality.md`](coding/testing_and_quality.md), [`startup.md`](runtime/startup.md), [`run_tests.bat`](../../app/tests/run_tests.bat) | Preserve the phase summary on future runs and investigate any new skip, failure, or warning. |
...
| --- | --- | --- | --- |
| `application.startup` | Medium | Direct launcher option 1 browser-opening behavior and explicit option 13 process-stop smoke. | Medium |
