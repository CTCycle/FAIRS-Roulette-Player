...
| `PARTIAL` | `backend.inference`, `frontend.workflows`, `validation.campaign` |
...
| `qa.standard-runner` | `VALIDATED` | Repository-standard dependency setup, live services, Python tests, and optional frontend test phases. | The canonical `app/tests/run_tests.bat` attempt stopped before collection because its hard-coded `runtimes/cache/uv` path returned access denied. An equivalent live Windows run with the same managed venv, services, pytest target, and a fresh repository-local cache recorded `208 passed, 8 skipped, 1 warning`; frontend lint and production build also passed. | The skipped phases and tests are explicit validation debt, not hidden passes; the warning is a dependency deprecation warning. The canonical pytest/uv cache requires an elevated token or a fresh cache root on this host. | — | 2026-09-21 | `unit + E2E` | [`testing_and_quality.md`](coding/testing_and_quality.md), [`startup.md`](runtime/startup.md), [`run_tests.bat`](../../app/tests/run_tests.bat), [`../QA/launcher-state-validation-20260921.md`](../QA/launcher-state-validation-20260921.md), [`../QA/python314-migration-validation-20260921.md`](../QA/python314-migration-validation-20260921.md) | Use the supported runner and its canonical cache when permissions allow; otherwise use a repository-local cache and investigate any new skip, failure, or warning. |
| `validation.campaign` | `PARTIAL` | Tiered validation of the core Windows + SQLite + CPU product profile, with conditional PostgreSQL, GPU, mixed-precision, JIT, maintenance, and resilience gates. | The campaign map and execution contract are tracked below. VAL-00 and VAL-01 are validated with real dataset/checkpoint and launcher/browser evidence; the first-tier startup animation regression is also covered by the focused E2E and refreshed screenshots. | Live checkpoint-backed inference, replay mutation, broader visual/accessibility coverage, device-dependent execution, and repeated-operation evidence remain open. | — | 2026-09-21 | `manual + E2E` | [`testing_and_quality.md`](coding/testing_and_quality.md), [`startup.md`](runtime/startup.md), [`workflows.md`](operations/workflows.md), [`findings_and_remediation.md`](architecture/findings_and_remediation.md), [`../QA/validation-campaign-tier0-20260921.md`](../QA/validation-campaign-tier0-20260921.md) | Continue VAL-02 and the fixture-backed inference campaign. |
...
The user-supplied comprehensive roadmap is digested here as the canonical campaign map; it does not create a second tracker. The current source baseline for this campaign is the `develop` checkout at `f61507d846bcea5c3349c35041252ff5b4d539de` before this documentation change. A slice status is promoted only from scenarios actually executed at a recorded revision.
...
| 1 — Foundations | `VAL-02`–`VAL-06` | Routing/layout, HTTP/generated contracts, runtime settings, SQLite persistence, and dataset import/lifecycle. | Mixed: contracts/settings/SQLite are validated; layout and valid-XLSX coverage are partial. | Run after Tier 0, prioritizing valid XLSX and below-minimum desktop behavior. |
| 2 — Core training | `VAL-07`–`VAL-10` | Wizard validation, short real training/checkpoint publication, cancellation/concurrency, resume/provenance handoff. | Training backend is validated; wizard/checkpoint UI evidence is partial. | Use the VAL-00 lineage and dependency-local regression. |
...
The recommended order is `VAL-00 → VAL-01 → VAL-02 → VAL-04 → VAL-06 → VAL-07 → VAL-08 → VAL-10 → VAL-11 → VAL-12 → VAL-13 → VAL-14 → VAL-15 → VAL-16 → VAL-17 → VAL-19 → VAL-21 → VAL-22`. `VAL-03`, `VAL-05`, `VAL-09`, `VAL-18`, and `VAL-20` are conditional or low-cost gates whose evidence must still be rechecked when their dependencies change. Packaged distribution remains outside scope.
...
| `validation.campaign` | Medium | Complete the ordered VAL-02–VAL-22 register and keep exact revision/evidence records for every slice. | High |
