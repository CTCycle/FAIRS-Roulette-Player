...
| `VAL-01` | Frontend-first startup, health polling/retry, owned shutdown, and startup visual state | Confirmed in launcher, `useBackendStartup`, startup components, and existing E2E coverage. | `VALIDATED` | Official launcher option 1; frontend ready on `8051`; in-app browser reached `/training`; `Connected` rendered; no startup screen after health; zero rendered as a full green conic sector rather than a card; 5 focused startup E2E tests; official elevated option 13 stopped nine owned processes and ports were rechecked. | None. | — | Replaced the positioned zero-pocket card with a green 9.73-degree rotor sector; no launcher remediation was required after the permission-safe rerun. | `TestStartupFlow`: `5 passed`, `7 deselected`; client build passed; startup screenshots show the full green sector; no browser console errors; ports `8890` and `8051` clear. | Tier 0 startup and shutdown evidence is complete. | 2026-09-21 / working tree rooted at `f61507d` / Windows, ports `8890`/`8051`. |
