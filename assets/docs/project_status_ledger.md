...
| `application.startup` | `VALIDATED` | Windows launcher preparation, database startup, backend health, frontend readiness, single-worker lifecycle, and stop behavior. | Standard runner, direct launcher option 1, exact backend-stop simulation with the frontend still serving, option 13 process cleanup, delayed startup browser inspection, automatic readiness transition, retry/reload flows, supported viewport checks, reduced-motion checks, and production build passed. | Browser opening remains best-effort by design; the runner's standalone frontend unit/E2E phases remain skipped because those npm scripts do not exist. | — | 2026-09-20 | `manual + E2E` | [`startup.md`](runtime/startup.md), [`deployment.md`](runtime/deployment.md), [`test_app_entrypoint.py`](../../app/tests/unit/test_app_entrypoint.py), [`app_flow.py`](../../app/tests/e2e/test_app_flow.py) | Re-run the launcher smoke and startup browser slice after future launcher or bootstrap changes. |
...
| `qa.standard-runner` | `VALIDATED` | Repository-standard dependency setup, live services, Python tests, and optional frontend test phases. | `app/tests/run_tests.bat` completed with `Live server phase: PASS`, `Python tests: PASS`, `Frontend bootstrap: PASS`, `Frontend unit tests: SKIPPED`, and `Frontend E2E tests: SKIPPED`; pytest reported `201 passed, 10 skipped, 1 warning in 65.61s`. | The skipped phases and tests are explicit validation debt, not hidden passes; the warning is a dependency deprecation warning. | — | 2026-09-20 | `unit + E2E` | [`testing_and_quality.md`](coding/testing_and_quality.md), [`startup.md`](runtime/startup.md), [`run_tests.bat`](../../app/tests/run_tests.bat) | Preserve the phase summary on future runs and investigate any new skip, failure, or warning. |
...
| --- | --- | --- | --- |
| `application.startup` | Low | Automatic browser-opening behavior is best-effort and not independently asserted; future launcher changes still need the direct smoke and startup browser slice. | Low |
