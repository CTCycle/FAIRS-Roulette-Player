...
- If local startup fails, run option 4 in `start_on_windows.ps1` and choose `Standard` to resync runtimes and application dependencies.
- If test or browser tooling is missing, run option 4 and choose `Development` so the server's `test` extra is installed.
- If ports are already occupied, resolve the conflicting listeners before relaunching.
- If the backend starts without the frontend, verify that `app/client/node_modules` contains Vite and that `app/client/dist/index.html` and its generated assets exist; retry option 1 for automatic recovery, run option 5 for a frontend-only rebuild, or run option 4 when frontend dependencies also need updating.
- Option 4 stops a process listening on the configured `UI_PORT` before `npm ci` replaces frontend dependencies. If npm still reports `EPERM` for `esbuild.exe`, stop any external Vite or Node process using `app/client/node_modules` and retry.
...
- If PostgreSQL mode is enabled, select option 6 in `start_on_windows.ps1` to create or upgrade the configured database. FastAPI startup runs the same idempotent runner. A missing target is created only for SQLSTATE `3D000`, and the configured role needs `CREATEDB`.
