...
- If the frontend reports a readiness failure after the backend became healthy, the launcher cleans up the processes it started. Check whether the configured UI port is owned by an unrelated process and verify that `app/client/dist` is complete.
