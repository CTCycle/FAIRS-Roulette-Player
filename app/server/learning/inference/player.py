...
from server.common.constants import STATES
...
class RoulettePlayer:
    ...
    def __init__(
        ...
        self.fixed_strategy_id = validate_strategy_id(
    ...
    def softmax(values: np.ndarray) -> np.ndarray:
        ...
        if not np.isfinite(denom) or denom <= 0:
            raise ValueError("Model logits cannot be normalized.")
    ...
    def predict_strategy(
        ...
        if self.strategy_model is None:
            raise RuntimeError("Configured strategy model is unavailable.")
    ...
    def predict_next(self) -> dict[str, Any]:
        ...
        gain_value = float(self.current_capital) / float(self.initial_capital)
        ...
        if logits.size != STATES:
            raise ValueError(
                "Model returned an invalid output size: "
        ...
        relative_preference = float(self.softmax(logits)[self.next_action])
        ...
        if self.dynamic_betting_enabled:
            ...
            prediction["current_bet_amount"] = self.bet_amount
    ...
    def update_bet_amount(
        ...
        self.fixed_strategy_id = validate_strategy_id(
            int(self.configuration["bet_strategy_fixed_id"])
        )
