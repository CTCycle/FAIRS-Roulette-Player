...
class Datasets(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "dataset_kind", "dataset_name_key", name="uq_datasets_kind_name_key"
        ),
        Index("ix_datasets_kind_name", "dataset_kind", "dataset_name_key"),
        CheckConstraint(
            "dataset_kind IN ('training', 'inference')", name="ck_datasets_kind"
        ),
    )
...
class DatasetOutcomes(Base):
    ...
    dataset_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("datasets.dataset_id", ondelete="CASCADE"), primary_key=True
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "outcome_id >= 0 AND outcome_id <= 36", name="ck_dataset_outcomes_outcome"
        ),
...
class InferenceSessions(Base):
    ...
    session_id: Mapped[str] = mapped_column(
        String(MAX_SESSION_ID_LENGTH), primary_key=True
    )
    dataset_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("datasets.dataset_id", ondelete="CASCADE"), nullable=False
    )
    checkpoint_name: Mapped[str] = mapped_column(
        String(MAX_CHECKPOINT_NAME_LENGTH), nullable=False
    )
    ...
    dataset: Mapped[Datasets] = relationship(
        back_populates="inference_sessions", lazy="raise"
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "initial_capital > 0", name="ck_inference_sessions_initial_capital"
        ),
...
class InferenceSessionSteps(Base):
    ...
    session: Mapped[InferenceSessions] = relationship(
        back_populates="steps", lazy="raise"
    )
    __table_args__ = (
