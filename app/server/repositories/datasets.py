...
class DatasetRepository:
    ...
    def list(self, dataset_kind: str | None = None, *, limit: int | None = None, offset: int = 0) -> list[dict[str, Any]]:
        with self.database.Session() as session:
            ...
            stmt = stmt.order_by(Datasets.dataset_kind, Datasets.dataset_name_key, Datasets.dataset_id).offset(max(offset, 0))
    ...
    def import_replacement(self, dataset_name: str, dataset_kind: str, outcomes: pd.DataFrame) -> dict[str, Any]:
        ...
        records = outcomes[["sequence_index", "outcome_id"]].astype(int).to_dict(orient="records") if not outcomes.empty else []
        now = datetime.now(timezone.utc)
        with self.database.transaction() as session:
            dataset = session.scalar(select(Datasets).where(Datasets.dataset_kind == clean_kind, Datasets.dataset_name_key == self._key(clean_name)).with_for_update())
            if dataset is None:
                dataset = Datasets(dataset_name=clean_name, dataset_name_key=self._key(clean_name), dataset_kind=clean_kind, created_at=now, updated_at=now)
                session.add(dataset)
                session.flush()
            else:
                dataset.dataset_name = clean_name
                dataset.updated_at = now
            session.execute(delete(DatasetOutcomes).where(DatasetOutcomes.dataset_id == dataset.dataset_id))
