...
def test_delete_dataset_delegates_to_cascade_repository() -> None:
    datasets = Mock()
    store = DataStore(datasets=datasets, inference=Mock())

    store.delete_dataset(7)
...
def test_clear_inference_session_steps_delegates_to_repository() -> None:
    inference = Mock()
    store = DataStore(datasets=Mock(), inference=inference)

    store.clear_inference_session_steps("session_1")
...
def test_delete_inference_session_delegates_to_repository() -> None:
    ...
    store.delete_inference_session("session_2")
